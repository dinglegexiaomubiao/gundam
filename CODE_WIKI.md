...
**匹配判定（`match`）** — 以「攻击型最高伤害武器的伤害类型」为轴，**同时对上两侧**：

| 侧 | 判定内容 |
|---|---|
| 支援型 | 该伤害类型有没有对应的「损伤提升」特效（`support_hit` / `support_by` 记来源特效名） |
| 防御型 | 该伤害类型有没有对应的减伤（`defense_hit` / `defense_by` 记来源能力名与百分比） |
...
- 集合相交即算命中，因此双类型武器（如 洗牌同盟拳 EX = 物理+特殊）
  只要支援覆盖其中之一就会命中。
...
**只要没覆盖全，就必须点名缺的是哪几类损伤提升**（不能只说"无法匹配"）：

| `level` | 含义 | `detail` 句式 |
|---|---|---|
| `full` | 完全覆盖 | `支援提供 物理、特殊损伤提升，完全覆盖攻击武器「洗牌同盟拳 EX」的 物理、特殊：2/2` |
| `partial` | 部分覆盖 | `支援提供 物理损伤提升，攻击武器「洗牌同盟拳 EX」为 物理、特殊；命中 物理，未命中 特殊：1/2` |
| `none` | 完全没命中 | `支援提供 光束损伤提升，但攻击型最高伤害武器「洗牌同盟拳 EX」为 物理、特殊，无法匹配：0/2（缺少 物理、特殊 损伤提升）` |
...
1. **移动力是「全队一条」而非「每台一条」** —— 原先每台一条，卡片上会出现一排
   `移动力 5/5`，看不出"全队都达标"。现在合成一条，`actual` = 全队最差值（决定 `ok`）、
   `best` = 最高值（决定 `bonus`），逐台明细放 `units`；
...
- `items[].detail`：**可直接展示的说明句**（如「全队移动力均为 5，达到及格线」
  /「对攻击型生效的特效「物理损伤提升（1回合）LV4」射程 5，达到及格线」），
  前端优先渲染它而不是干巴巴的 `5/5`；
- `items[].bonus`：该项**超过**及格线（前端加 `★` 与金色边框）；
- `bonuses[]`：不参与及格线计数的纯加分项，目前有三类：
  | key | 内容 |
  |---|---|
